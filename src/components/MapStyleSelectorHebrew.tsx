...
interface MapStyleSelectorHebrewProps {
  currentStyle: string;
...
const MapStyleSelectorHebrew: React.FC<MapStyleSelectorHebrewProps> = ({ currentStyle, onStyleChange }) => {
    ...
    <TooltipProvider>
      <div className="relative" ref={dropdownRef} dir="rtl">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              className={`
                ...
                p-2 min-w-[120px] h-12 text-right
              `}
            >
          ...
          <TooltipContent side="left" className="bg-black/90 text-white text-sm text-right" dir="rtl">
            <p>נבחר כעת: {currentStyleData.nameHebrew}</p>
            <p className="text-xs opacity-75">{currentStyleData.descriptionHebrew}</p>
          </TooltipContent>
        </Tooltip>
        ...
        `}>
          <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
            {/* Header */}
            <div className="p-4 border-b border-gray-100" dir="rtl">
              <h3 className="text-lg font-semibold text-gray-800 mb-2 text-right">סגנונות מפה</h3>
              
              {/* Category filters */}
              <div className="flex gap-1 overflow-x-auto justify-end flex-row-reverse">
                <button
                ...
                {Object.entries(categoryIcons).map(([category, Icon]) => (
                  <button
                    ...
                    `}
                  >
                    <span>{categoryLabelsHebrew[category as keyof typeof categoryLabelsHebrew]}</span>
                ...
                {filteredStyles.map((style, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        group relative p-3 rounded-xl border-2 text-right
                        transition-all duration-300 ease-out
                    ...
                    >
                      ...
                      <div className="relative mb-2 overflow-hidden rounded-lg">
                        <img
                          src={style.preview}
                          alt={`${style.nameHebrew} תצוגה מקדימה`}
                          className={`
...
export default MapStyleSelectorHebrew; 